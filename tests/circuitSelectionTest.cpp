...
TEST_F(CircuitSelectionTest, InvalidConnections) {
	for (i = 0; i < 100; i++) {
		Position pos1(rand() % 100000, rand() % 100000);
		Position pos2(rand() % 100000, rand() % 100000);
		...
		bool selfConnect = circuit->tryCreateConnection(
			std::make_shared<CellSelection>(pos1), std::make_shared<CellSelection>(pos1)
		);
...
TEST_F(CircuitSelectionTest, ConnectionRemoval) {
	for (i = 0; i < 100; i++) {
		Position pos1(rand() % 100000, rand() % 100000);
		Position pos2(rand() % 100000, rand() % 100000);
		if (pos1 == pos2) {
		...
		circuit->tryCreateConnection(
			std::make_shared<CellSelection>(pos1), std::make_shared<CellSelection>(pos2)
		);
...
TEST_F(CircuitSelectionTest, BlockConnectionRemoval) {
	for (int i = 0; i < 100; i++) {
		Position pos1(rand() % 100000, rand() % 100000);
		Position pos2(rand() % 100000, rand() % 100000);
		if (pos1 == pos2) {
		...
		circuit->tryCreateConnection(std::make_shared<CellSelection>(pos1), std::make_shared<CellSelection>(pos2));
		circuit->tryCreateConnection(std::make_shared<CellSelection>(pos2), std::make_shared<CellSelection>(pos1));
...
TEST_F(CircuitSelectionTest, MoveBlockSimple) {
	...
	const Block* block1 = container.getBlock(pos1);
	bool moved = circuit->tryMoveBlocks(
		std::make_shared<CellSelection>(pos1),
		pos2-pos1,
		Orientation()
	);
...
TEST_F(CircuitSelectionTest, MoveBlock) {
	Position pos1(0, 0);
	Position pos2(10, 10);
	...
	circuit->tryMoveBlocks(
		std::make_shared<CellSelection>(pos2),
		pos4-pos2,
		Orientation()
	);
