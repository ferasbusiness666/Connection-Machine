...
TEST_F(WeirdCasesEvaluatorTest, InitializationBehaviorWithICs) {
	...
	EXPECT_EQ(evaluator->getState(andPos), logic_state_t::LOW);
	evaluator->tickStep(1);
	EXPECT_EQ(evaluator->getState(xnorPos), logic_state_t::HIGH);

	evaluator->setState(andPos, logic_state_t::HIGH);
	EXPECT_EQ(evaluator->getState(andPos), logic_state_t::HIGH);
	evaluator->tickStep(1);
	EXPECT_EQ(evaluator->getState(xnorPos), logic_state_t::LOW);
	...
	EXPECT_EQ(evaluator2->getState(andPos), logic_state_t::LOW);
	evaluator2->tickStep(1);
	EXPECT_EQ(evaluator2->getState(xnorPos), logic_state_t::HIGH);

	evaluator2->setState(andPos, logic_state_t::HIGH);
	EXPECT_EQ(evaluator2->getState(andPos), logic_state_t::HIGH);
	evaluator2->tickStep(1);
