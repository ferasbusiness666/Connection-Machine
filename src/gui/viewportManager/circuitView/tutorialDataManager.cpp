...
void TutorialDataManager::initializeTutorials() {
	...
	for (const auto& file : std::filesystem::directory_iterator((DirectoryManager::getResourceDirectory() / "tutorials" / "").string())) {
		auto pair = tutorials.emplace(parseTutorialFile(file.path().string()));
...
void writeTutorial(const std::vector<TutorialStep>& steps, const std::string& tutorialName, const std::string& filepath) {
	std::ofstream out;
	out.open(filepath);
	out << "Tutorial: \"(NEW)" << tutorialName << "\"\n";
	for (int i = 0; i < steps.size(); i++) {
		out << STEP_TOKEN << "\n";
		out << "\t" << ACTION_TOKEN << "\n";
		TutorialStep cur = steps[i];
		...
		for (int j = 0; j < curCondition.blocks.size(); j++) {
			...
			out << "\t\t" << BLOCK_CONDITION_TOKEN << " " << blockTypeToString(curEl.type) << " " << curEl.pos.toString() << " " << orientationToString(curEl.orientation)
				<< "\n";
		}
