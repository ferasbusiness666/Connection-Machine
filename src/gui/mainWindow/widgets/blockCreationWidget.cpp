...
void BlockCreationWidget::render() {
	...
	if (ImGui::Begin(("Block Creation###" + getWidgetIdStr()).c_str(), &open)) {
		...
		{ // blockData
			...
			if (ImGui::BeginChild("Block Data", ImVec2(ImGui::GetContentRegionAvail().x, 100), ImGuiChildFlags_ResizeY | ImGuiChildFlags_AlwaysUseWindowPadding)) {
				ImGui::PopStyleColor();
				ImGui::PopStyleVar();
				{
					ImGui::Text("Name");
		...
		{ // renderDataList
			ImGui::PushStyleVar(ImGuiStyleVar_ChildRounding, 4);
			ImGui::PushStyleColor(ImGuiCol_ChildBg, GUIColors::WIDGET_BACKGROUND);
			if (ImGui::BeginChild("Render Data", ImVec2(ImGui::GetContentRegionAvail().x, 300), ImGuiChildFlags_ResizeY | ImGuiChildFlags_AlwaysUseWindowPadding, ImGuiWindowFlags_MenuBar)) {
				ImGui::PopStyleColor();
				ImGui::PopStyleVar();
